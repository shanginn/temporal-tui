...
function __fish_temporal_tui_global_optspecs
    string join \n config= profile= address= n/namespace= api-key= tls tls-ca= tls-cert= tls-key= tls-server-name= header= codec-endpoint= codec-header= codec-auth= q/query= page-size= refresh-seconds= no-auto-refresh no-color read-only web-ui-url= h/help V/version
end
